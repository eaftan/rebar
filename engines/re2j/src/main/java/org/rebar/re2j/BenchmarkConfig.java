...
record BenchmarkConfig(
  ...
  static BenchmarkConfig parse(byte[] input) throws CharacterCodingException {
    ...
    for (int offset = 0; offset < input.length; ) {
      ...
      switch (entry.key()) {
        ...
        case "pattern" -> {
          if (pattern != null) {
            throw new IllegalArgumentException("RE2/J does not support multiple patterns");
          }
  ...
  Pattern compilePattern(String regex) {
    ...
    // RE2/J always uses Unicode case folding and ASCII Perl character classes.
    // There is no flag corresponding to Rebar's Unicode toggle.
